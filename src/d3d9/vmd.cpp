...
public:
		...
		int export_mode)
	{
		...
		for (int i = 0; i < pmd_num; ++i) {
			...
			if (file_path_map.find(filename) != file_path_map.end()) {
				continue;
			}
			...
			if (pmd)
			{
				...
				file_path_map[filename] = data_list.size() - 1;
			}
			else
			{
	...
	bool execute_vmd_export(int currentframe)
	{
		...
		if (currentframe == BridgeParameter::instance().start_frame)
		{
			...
			{
				const char* filename = ExpGetPmdFilename(i);
				FileDataForVMD& file_data = data_list.at(file_path_map[filename]);
	...
	bool write()
	{
		...
		for (int i = 0; i < pmd_num; ++i)
		{
			...
			if (file_data.vmd)
			{
				...
				auto output_filepath = umbase::UMStringUtil::utf16_to_wstring(umbase::UMStringUtil::utf8_to_utf16(output_path) + filename);
...
private:
	void init_file_data(FileDataForVMD& data)
	{
		if (data.pmd) {
